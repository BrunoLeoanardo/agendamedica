...
public class Agendamento {
    ...
    public void pesquisar(String cpf){
        for(int i=0; i<pacientes.size();i++){
            if(pacientes.get(i).getCpf(). equals(cpf)){
                ...
                System.out.println(pacientes.get(i).getEmail());
                i=pacientes.size();
            }
        }
    }
    ...
    public void apresentarListaDePacientes(){
        System.out.println("Data do atendimento:"+data+", Medico(a):"+medico.getNome()+", "+medico.getEspecialidade());
        for (int i=0; i<pacientes.size(); i++){
            Paciente p  = pacientes.get(i);
            System.out.println(i+1+","+p.getNome()+", "+p.getCpf()+", "+p.getEmail());
        }
    }
}
